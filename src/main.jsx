...
function App() {
  ...
  if (!session) {
    return (
      <div className="auth-page"><div className="auth-glow" /><div className="auth-card">
        <div className="brand"><div className="logo-mark">C</div><div><h1>Convogram</h1><span>Connect. Share. Communicate.</span></div></div>
        <div className="auth-heading"><span className="eyebrow">THE SOCIAL SUPERAPP</span><h2>{authMode === "login" ? "Welcome back" : "Join Convogram"}</h2><p>{authMode === "login" ? "Your people, conversations and moments are waiting." : "One place for messages, Moments, Shorts and communities."}</p></div>
        <form onSubmit={handleAuth}>
          {authMode === "signup" && <><label>Display name</label><input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Your name" /><label>Username</label><input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="username" /></>}
          <label>Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" />
          <label>Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Your password" />
        ...
        <div className="auth-switch"><span>{authMode === "login" ? "New to Convogram?" : "Already here?"}</span><button onClick={() => setAuthMode(authMode === "login" ? "signup" : "login")}>{authMode === "login" ? "Create account" : "Log in"}</button></div>
      </div></div>
    );
  }
  ...
  return (
    <div className="app-shell">
      <header className="topbar"><button className="topbar-brand" onClick={() => setActiveTab("home")}><div className="small-logo">C</div><strong>Convogram</strong></button><div className="topbar-actions"><button onClick={() => setSearchOpen((v) => !v)} aria-label="Search"><Search size={20} /></button><button aria-label="Notifications"><Bell size={20} /></button><button onClick={() => setActiveTab("profile")} aria-label="Profile"><div className="mini-avatar">{avatarLetter(profile)}</div></button></div></header>
      {searchOpen && <div className="search-bar"><Search size={18} /><input autoFocus value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search Convogram..." /><button onClick={() => { setSearchTerm(""); setSearchOpen(false); }}><X size={18} /></button></div>}
      <main className="main-content">
        {error && <div className="global-error">{error}<button onClick={() => setError("")}><X size={15} /></button></div>}
        {activeTab === "home" && <>
          <section className="hero-card"><div><span className="eyebrow">GOOD TO SEE YOU, {profile?.display_name?.split(" ")[0]?.toUpperCase() || "FRIEND"}</span><h1>Everything you love.<br /><span>One conversation.</span></h1><p>Share Moments, discover Shorts, message your people and build communities.</p><div className="hero-actions"><button onClick={() => openComposer("post")}><Plus size={17} /> Create post</button><button className="ghost" onClick={() => openComposer("moment")}><Camera size={17} /> Add Moment</button></div></div><div className="hero-orbit"><Sparkles size={34} /><div>Connect<br />more.</div></div></section>
          <section className="section"><div className="section-header"><div><span className="eyebrow">24 HOURS</span><h2>Moments</h2></div><button onClick={() => openComposer("moment")}><Plus size={17} /> Add</button></div><MomentsRow moments={moments} currentUserId={session.user.id} onAddMoment={() => openComposer("moment")} onViewMoment={viewMoment} loading={momentLoading} error={null} /></section>
          <section className="section"><div className="section-header"><div><span className="eyebrow">YOUR FEED</span><h2>For you</h2></div><button onClick={() => refreshHome(session.user.id)}><Compass size={16} /> Refresh</button></div>{feedLoading ? <div className="feed-loader">Loading your feed...</div> : filteredPosts.length ? filteredPosts.map((post) => <PostCard key={post.id} post={post} currentUserId={session.user.id} isLiked={!!likedPosts[post.id]} onLike={() => handleLike(post)} onUnlike={() => handleUnlike(post)} onComment={(content) => handleComment(post, content)} onDelete={() => handleDelete(post)} likeCount={post.likes?.[0]?.count || 0} commentCount={post.comments?.[0]?.count || 0} />) : <div className="empty-state"><Sparkles size={28} /><h2>Your feed is ready</h2><p>Be the first to share something with your Convogram community.</p><button className="primary-button small" onClick={() => openComposer("post")}>Create your first post</button></div>}</section>
        </>}
        {activeTab === "shorts" && <section className="page-section"><div className="page-title"><div><span className="eyebrow">DISCOVER</span><h1>Shorts</h1></div><button className="round-button"><Video size={20} /></button></div><div className="shorts-grid">{shorts.length ? shorts.map((short) => <article className="short-card" key={short.id}><video src={short.video_url || short.media_url} controls playsInline /><div className="short-info"><strong>@{short.profiles?.username || "creator"}</strong><span>{short.caption || ""}</span></div></article>) : <div className="empty-state"><Zap size={30} /><h2>Shorts are coming to life</h2><p>Short videos from creators you follow will appear here.</p></div>}</div></section>}
      ...
      <nav className="bottom-nav">{navItems.map(({ id, label, icon: Icon }) => <button key={id} className={`nav-button ${activeTab === id ? "active" : ""}`} onClick={() => setActiveTab(id)}><Icon size={21} /><small>{label}</small></button>)}</nav>
