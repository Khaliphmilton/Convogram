...
const avatar = (profile) => (profile?.display_name || profile?.username || "C").slice(0, 1).toUpperCase();
...
function App() {
  ...
  const openAccount = (account) => { setProfile(account); setStats({ postsCount: 0, followersCount: 0, followingCount: 0 }); setActive("profile"); };
  return <div className="app" style={{ overflowX: "hidden" }}>
    <header className="topbar"><button className="brand-button" onClick={() => setActive("home")}><span className="brand-mark small">C</span><b>Convogram</b></button><div className="top-search" /><div className="top-actions"><button className="top-create" onClick={() => openComposer("post")}><Plus size={20} /><span>Post</span></button><button onClick={() => setActive("search")}><Search size={20} /></button><button onClick={() => setFeatureSuite(true)}><SlidersHorizontal size={20} /></button><button className="notification" onClick={openNotifications}><Bell size={20} />{unread > 0 && <i>{unread > 9 ? "9+" : unread}</i>}</button><button onClick={() => setActive("profile")}><span className="avatar mini">{avatar(profile)}</span></button></div></header>
    {notifications && <NotificationsPanel userId={session.user.id} onClose={() => { closeNotifications(); refresh(); }} />}
    <main className="layout"><aside className="sidebar"><div className="side-profile"><span className="avatar">{avatar(profile)}</span><div><b>{profile?.display_name || "You"}</b><small>@{profile?.username || "user"}</small></div></div>{nav.map(([id, label, Icon]) => <button key={id} className={active === id ? "active" : ""} onClick={() => setActive(id)}><Icon size={20} /><span>{label}</span>{id === "messages" && <em>Chat</em>}</button>)}<div className="sidebar-divider" /><button className="create-nav" onClick={() => openComposer("post")}><Plus size={20} /><span>Create post</span></button><button onClick={() => setActive("search")}><Search size={20} /><span>Search</span></button><button onClick={() => setFeatureSuite(true)}><SlidersHorizontal size={20} /><span>All features</span></button><button onClick={() => setActive("settings")}><Settings size={20} /><span>Settings</span></button><button className="logout" onClick={logout}><span>↪</span><span>Log out</span></button></aside>
      <section className="content">{error && <div className="global-alert">{error}<button onClick={() => setError("")}><X size={15} /></button></div>}
        {active === "home" && <><section className="panel first-panel"><div className="section-head"><div><small>24 HOURS</small><h2>Moments</h2></div><div className="section-actions"><button onClick={() => openComposer("moment")}><Plus size={16} /> Add Moment</button><button onClick={() => openComposer("post")}><Plus size={16} /> Post</button></div></div><MomentsRow moments={moments} currentUserId={session.user.id} onAddMoment={() => openComposer("moment")} loading={false} error={null} /></section><section className="panel"><div className="section-head"><div><small>YOUR FEED</small><h2>For you</h2></div><div className="section-actions"><button onClick={() => openComposer("post")}><Plus size={16} /> Create post</button><button onClick={refresh}><Compass size={16} /> Refresh</button></div></div>{loadingFeed ? <div className="empty">Loading your feed…</div> : posts.length ? posts.map((post) => <PostCard key={post.id} post={post} currentUserId={session.user.id} isLiked={!!liked[post.id]} onLike={() => toggleLike(post)} onUnlike={() => toggleLike(post)} onComment={(text) => comment(post, text)} onDelete={() => removePost(post)} likeCount={post.likes?.[0]?.count || 0} commentCount={post.comments?.[0]?.count || 0} />) : <div className="empty"><Sparkles size={28} /><h3>Your feed starts here.</h3><p>Share the first thing your people will see.</p><button className="primary" onClick={() => openComposer("post")}>Create your first post</button></div>}</section></>}
        {active === "search" && <SearchPage currentUserId={session.user.id} onBack={goBack} onOpenProfile={openAccount} />}
        {active === "shorts" && <section className="page"><div className="page-head"><div><small>SHORT VIDEO</small><h1>Shorts</h1><p>Short videos, sounds and creators.</p></div><button onClick={loadShorts}><Zap size={18} /> Refresh</button></div><ShortsPanel shorts={shorts} userId={session.user.id} /></section>}
    ...
    <nav className="mobile-nav">{nav.map(([id, label, Icon]) => <button key={id} className={active === id ? "active" : ""} onClick={() => setActive(id)}><Icon size={21} /><span>{label}</span></button>)}</nav>
