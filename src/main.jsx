...
function App() {
  ...
  const [profileStats, setProfileStats] = useState({ postsCount: 0, followersCount: 0, followingCount: 0 });
  ...
  async function refreshAccountData(userId) {
    try {
      const [stats, unread] = await Promise.all([getProfileStats(userId), getUnreadNotificationsCount(userId)]);
      setProfileStats(stats || { postsCount: 0, followersCount: 0, followingCount: 0 });
      setUnreadNotifications(unread || 0);
    } catch (err) { setError(err.message || "Unable to refresh account data."); }
  }

  async function loadSession() {
    if (!supabase) { setError("Convogram is not connected to Supabase yet."); setLoading(false); return; }
  ...
  async function refreshHome(userId) {
    setFeedLoading(true); setMomentLoading(true);
    try {
      const [feed, activeMoments] = await Promise.all([getFeed(30), getMomentsForFeed(userId, 30)]);
      setPosts(feed || []); setMoments(activeMoments || []);
      const likes = {};
      await Promise.all((feed || []).map(async (post) => { likes[post.id] = await isPostLikedByUser(post.id, userId); }));
      setLikedPosts(likes);
    } catch (err) { setError(err.message || "Unable to load your feed."); }
    finally { setFeedLoading(false); setMomentLoading(false); }
  }

  async function loadShorts() {
    try { setShorts((await getShortsForDiscover(20, 0)) || []); }
    catch (err) { setError(err.message || "Unable to load Shorts."); }
  }
  ...
  async function handleAuth(e) {
    ...
    if (authMode === "login") {
      ...
      setSession(data.session); if (data.session) await loadProfile(data.session.user.id); return;
    }
  ...
  async function logout() {
    await supabase.auth.signOut(); setSession(null); setProfile(null); setPosts([]); setMoments([]); setActiveTab("home");
  }
  ...
  async function publish() {
    ...
    try {
      if (composerType === "post") {
        ...
        if (selectedFile) { const uploaded = await uploadPostMedia(selectedFile, session.user.id); mediaUrl = uploaded.url; mediaType = uploaded.mediaType; }
      ...
      closeComposer(); await refreshAccountData(session.user.id);
    } catch (err) { setError(err.message || "Publishing failed."); }
  ...
  {activeTab === "profile" && <section className="profile-page"><div className="profile-cover"><div className="cover-glow" /></div><div className="profile-main"><div className="profile-avatar">{avatarLetter(profile)}</div><button className="edit-profile" onClick={() => setMessage("Profile editing is ready for the next profile-settings surface.")}><Settings size={15} /> Edit</button><h1>{profile?.display_name || "Convogram User"}</h1><p className="username">@{profile?.username || "user"}</p><p className="bio">{profile?.bio || "Welcome to my Convogram profile."}</p><div className="profile-stats"><div><strong>{profileStats.postsCount}</strong><span>Posts</span></div><div><strong>{profileStats.followersCount}</strong><span>Followers</span></div><div><strong>{profileStats.followingCount}</strong><span>Following</span></div></div><div className="profile-actions"><button onClick={() => openComposer("post")}><Plus size={16} /> Post</button><button onClick={() => openComposer("moment")}><Camera size={16} /> Moment</button></div><button className="logout-button" onClick={logout}><LogOut size={16} /> Log out</button></div></section>}
