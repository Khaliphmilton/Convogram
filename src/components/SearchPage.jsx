...
export function SearchPage({ currentUserId, onBack, onOpenProfile }) {
  ...
  return <section className="search-page">
    ...
    <div className="accounts-list">{loading ? <div className="search-empty">Loading accounts…</div> : filtered.length ? filtered.map((person) => <button className="account-row" key={person.id} onClick={() => onOpenProfile?.(person)}><div className="account-avatar">{person.avatar_url ? <img src={person.avatar_url} alt=""/> : <span>{(person.display_name || person.username || "C").slice(0,1).toUpperCase()}</span>}</div><div className="account-info"><strong>{person.display_name || person.username || "Convogram User"}</strong><span>@{person.username || "user"}</span>{person.bio && <small>{person.bio}</small>}</div>{person.id !== currentUserId && <UserPlus size={18}/>}</button>) : <div className="search-empty"><Search size={30}/><h2>No accounts found</h2><p>Try another name or username.</p></div>}</div>
  </section>;
}
