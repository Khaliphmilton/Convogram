...
export function CallOverlay({ conversationId, userId, remoteUserId, remoteName = "Contact" }) {
  ...
  useEffect(() => {
    ...
    channel.on("broadcast", { event: "signal" }, async ({ payload }) => {
      ...
      try {
        ...
        setError(err.message || "Call connection failed.");
      }
    }).subscribe();
    return () => {
  ...
  async function signal(payload) {
    ...
    if (!channel) return;
    await channel.send({ type: "broadcast", event: "signal", payload: { ...payload, senderId: userId } });
  }

  function createPeer(callId, type, peerId) {
    const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS });
    pc.onicecandidate = event => {
      if (event.candidate) signal({ kind: "ice", callId, toUserId: peerId, candidate: event.candidate });
    };
    pc.ontrack = event => {
  ...
  async function startCall(type) {
    if (!remoteUserId || !conversationId) {
      setError("This conversation has no other participant to call.");
      return;
    }
    ...
    try {
      ...
      const pc = createPeer(callId, type, remoteUserId);
      stream.getTracks().forEach(track => pc.addTrack(track, stream));
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      setCall({ callId, type, status: "calling" });
      await signal({ kind: "offer", callId, type, toUserId: remoteUserId, offer });
    } catch (err) {
      cleanup(false);
  ...
  async function handleSignal(payload) {
    if (payload.kind === "offer") {
      ...
      setIncoming({ ...payload, fromUserId: payload.senderId });
      return;
    }
  ...
  async function acceptIncoming() {
    ...
    try {
      ...
      setCall({ callId: pending.callId, type: pending.type, status: "connected" });
      await signal({ kind: "answer", callId: pending.callId, type: pending.type, toUserId: pending.senderId, answer });
    } catch (err) {
      cleanup(false);
  ...
  async function declineIncoming() {
    if (!incoming) return;
    await signal({ kind: "hangup", callId: incoming.callId, toUserId: incoming.senderId });
    setIncoming(null);
  }

  async function hangup() {
    if (callRef.current?.callId) await signal({ kind: "hangup", callId: callRef.current.callId, toUserId: remoteUserId });
    cleanup(false);
  }
  ...
  return <>
    {(call || incoming) && <div className="call-layer">
      ...
      {call && <div className={`active-call ${call.type === "video" ? "video-call" : "voice-call"}`}>
        ...
        </> : <div className="voice-call-stage"><div className="call-avatar">{remoteName.slice(0, 1).toUpperCase()}</div><strong>{remoteName}</strong><span>{call.status === "calling" ? "Calling…" : "Connected"}</span><Volume2 size={18} /></div>}
    ...
    {error && !call && !incoming && <div className="call-toast" onClick={() => setError("")}>{error}</div>}
  </>;
}
