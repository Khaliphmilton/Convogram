...
function friendlyCommunityError(err) {
  ...
  if (message.includes("schema cache") || message.includes("Could not find the table 'public.communities'")) {
    return "Communities are being connected to the database. Please apply the latest Convogram Supabase migration, then refresh.";
  }
...
export function CommunitiesPanel({ userId }) {
  ...
  async function load() {
    ...
    } catch (err) { setError(friendlyCommunityError(err)); }
  ...
  return <div className="community-workspace">
    ...
    <header className="community-toolbar">
      <div className="community-heading"><span className="eyebrow">YOUR SPACES</span><h2>Communities</h2></div>
      <div className="community-toolbar-actions"><button className="secondary-button" onClick={load}><Compass size={16} /> Discover</button><button className="primary-button small" onClick={() => setCreateOpen((value) => !value)}><Plus size={16} /> Create</button></div>
    </header>

    {createOpen && <form className="community-create-inline" onSubmit={handleCreate}>
      <div className="community-create-copy"><span className="eyebrow">NEW SPACE</span><h3>Create community</h3><p>Start a space for people to connect and share.</p></div>
      <div className="community-create-fields"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Community name" required /><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What is this community about?" /><div className="community-create-actions"><button type="button" className="secondary-button" onClick={() => setCreateOpen(false)}>Cancel</button><button className="primary-button" type="submit">Create community</button></div></div>
    </form>}
