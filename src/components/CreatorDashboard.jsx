...
export function CreatorDashboard({ userId }) {
  ...
  return <section className="creator-dashboard"><div className="creator-dashboard-head"><div><span className="eyebrow">CREATOR STUDIO</span><h2>Your performance</h2></div><BarChart3 size={22} /></div><div className="creator-metrics"><div><Users /><strong>{stats.followers || 0}</strong><span>Followers</span></div><div><Heart /><strong>{engagement?.likes || 0}</strong><span>Likes</span></div><div><MessageCircle /><strong>{engagement?.comments || 0}</strong><span>Comments</span></div><div><Coins /><strong>{((earnings?.totalCents || 0) / 100).toFixed(2)}</strong><span>Earnings</span></div></div></section>;
}
