...
export function ProfileOptionsPage({ profile, email, onBack, onViewProfile, onSaved, onEditProfile, onSettings, onLogout }) {
  ...
  return <section className="page profile-options-page">
    ...
    {switcherOpen && <div className="account-switch-backdrop" onClick={() => setSwitcherOpen(false)}><div className="account-switcher" onClick={(e) => e.stopPropagation()}><div className="account-switch-head"><div><small>ACCOUNTS</small><h2>Switch account</h2></div><button onClick={() => setSwitcherOpen(false)} aria-label="Close"><X size={19}/></button></div><div className="saved-account-list">{savedAccounts.map((account) => <button key={account.id} className={`saved-account ${account.id === profile?.id ? "current" : ""}`} onClick={() => chooseAccount(account)}><span className="avatar saved-account-avatar">{(account.display_name || "C").slice(0,1).toUpperCase()}</span><span><b>{account.display_name}</b><small>@{account.username}</small></span>{account.id === profile?.id && <em>Current</em>}</button>)}</div>{selectedAccount && <div className="switch-password"><label>Enter password for @{selectedAccount.username}<input autoFocus type="password" value={switchPassword} onChange={(e) => setSwitchPassword(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") switchToAccount(); }} placeholder="Password" /></label>{switchError && <div className="switch-error">{switchError}</div>}<button className="primary" disabled={switching || !switchPassword} onClick={switchToAccount}>{switching ? "Switching…" : "Switch account"}</button></div>}<button className="add-account-button" onClick={() => { setSwitcherOpen(false); onLogout(); }}><span className="option-icon"><User size={17}/></span><span><b>Add account</b><small>Log in to another Convogram account</small></span><ChevronRight size={18}/></button><p className="account-switch-note">Saved account names and login emails stay on this device. Convogram never saves your passwords.</p></div></div>}
  </section>;
}
