...
export async function getDirectConversation(userId, otherUserId) {
  ...
  const { data: mine, error: mineError } = await supabase.from("conversation_members").select("conversation_id, conversations!inner(id, type, name, avatar_url, created_by, created_at, updated_at)").eq("user_id", userId).eq("conversations.type", "direct");
  if (mineError) throw mineError;
  const ids = (mine || []).map((row) => row.conversation_id).filter(Boolean);
  if (!ids.length) return null;
  const { data: theirs, error: theirsError } = await supabase.from("conversation_members").select("conversation_id").eq("user_id", otherUserId).in("conversation_id", ids);
...
export async function getConversationDetails(conversationId) {
  const { data, error } = await supabase.from("conversations").select(`*, profiles:created_by(id, username, display_name, avatar_url), conversation_members(*, profiles:user_id(id, username, display_name, avatar_url))`).eq("id", conversationId).single();
  if (error) throw error;
  return data;
}

export async function getMessages(conversationId, limit = 100, userId = null) {
  const { data, error } = await supabase.from("messages").select(`*, profiles:sender_id(id, username, display_name, avatar_url), message_reactions(*)`).eq("conversation_id", conversationId).order("created_at", { ascending: true }).limit(limit);
  if (error) throw error;
  ...
  const { data: replyMessages, error: replyError } = await supabase.from("messages").select(`id, content, message_type, media_url, is_deleted, sender_id, created_at, profiles:sender_id(id, username, display_name, avatar_url)`).in("id", replyIds);
...
export async function sendMessage(conversationId, senderId, content, messageType = "text", mediaUrl = null, replyToId = null) {
  const { data, error } = await supabase.from("messages").insert([{ conversation_id: conversationId, sender_id: senderId, content: messageType === "text" ? content : null, message_type: messageType, media_url: mediaUrl, reply_to_id: replyToId || null }]).select(`*, profiles:sender_id(id, username, display_name, avatar_url)`).single();
  if (error) throw error;
  if (replyToId) {
    const { data: reply } = await supabase.from("messages").select(`id, content, message_type, media_url, is_deleted, sender_id, created_at, profiles:sender_id(id, username, display_name, avatar_url)`).eq("id", replyToId).maybeSingle();
...
export function subscribeToConversation(conversationId, onInsert, onUpdate) {
  const channel = supabase.channel(`convogram-chat-${conversationId}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `conversation_id=eq.${conversationId}` }, (payload) => onInsert(payload.new)).on("postgres_changes", { event: "UPDATE", schema: "public", table: "messages", filter: `conversation_id=eq.${conversationId}` }, (payload) => onUpdate?.(payload.new)).subscribe();
...
export async function markMessageAsRead(messageId, userId) {
  ...
  const { data: message, error: messageError } = await supabase.from("messages").select("conversation_id, created_at").eq("id", messageId).maybeSingle();
  if (!messageError && message?.conversation_id) await supabase.from("conversation_members").update({ last_read_at: message.created_at }).eq("conversation_id", message.conversation_id).eq("user_id", userId);
  return data;
}
