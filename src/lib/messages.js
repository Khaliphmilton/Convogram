...
function installUnreadMessageIndicator() {
  ...
  const paint = async () => {
    ...
    try {
      ...
      navButtons.forEach(button => { if (total > 0) { const badge = document.createElement("span"); badge.className = badgeClass; badge.textContent = total > 99 ? "99+" : String(total); badge.setAttribute("aria-label", `${total} unread messages`); styleBadge(badge); button.appendChild(badge); } });
  ...
  const start = async () => { const { data } = await supabase.auth.getSession(); userId = data?.session?.user?.id || null; await paint(); if (timer) clearInterval(timer); timer = setInterval(paint, 3000); observer = new MutationObserver(() => { if (!document.querySelector(`.${badgeClass}`)) paint(); }); observer.observe(document.body, { childList: true, subtree: true }); };
  start();
