...
async function upload(file, path, mediaType) {
  ...
  return { url: publicUrl, path: data.path, mediaType };
}

export async function uploadPostMedia(file, userId) { return upload(file, `${userId}/${Date.now()}-${Math.random().toString(36).slice(2)}${getFileExtension(file.name)}`, validateMedia(file)); }
...
export async function uploadProfileAvatar(file, userId) { if (!ALLOWED_IMAGE_TYPES.includes(file.type)) throw new Error("File must be an image."); if (file.size > 5 * 1024 * 1024) throw new Error("Avatar file is too large. Maximum size is 5MB."); return (await upload(file, `avatars/${userId}/avatar${getFileExtension(file.name)}`, "image")).url; }
function getFileExtension(fileName) { const i = fileName.lastIndexOf("."); return i === -1 ? "" : fileName.substring(i); }
